...
describe('JSONPatch', () => {
  ...
  it('will use the JSON representation to add/replace a value', () => {
    ...
    obj = patch.apply(obj);
    expect(obj).to.deep.equal({ test: { name: 'test tester' }, test2: { name: 'test2 tester' } })
  })
  ...
  it('can copy a value', () => {
    ...
    obj = patch.apply(obj);
    expect(obj).to.deep.equal({ test: true, test2: true, uniqueValue: { name: 'test' }, anotherValue: { name: 'test' } });
  ...
  it('can update a delta text value', () => {
    ...
    obj = patch.apply(obj);
    expect(obj.text).to.deep.equal({ ops: [ { insert: 'This is my amazing text.\n' }]});
  })
  ...
  describe('invert', () => {
    ...
    it('will throw an error when the base object is incorrect', () => {
      ...
      expect(invert).to.throw('Patch mismatch. This patch was not applied to the provided object and cannot be inverted.');
    })
  })
})
